...
if (
  !dev_name ||
  dev_name.textContent.trim() != "TikTok: @7_night_music - OnePlus15"
) {
  localStorage.clear();
...
document.getElementById("phoneName").textContent =
  localStorage.getItem("phoneName") || "Сіздің телефоныңыз";
...
const formatted = now.toLocaleDateString("kk-kz", options);
...
target.innerText += "@7_night_music";
...
function openApp(idApp) {
  // idApp (1 - 12)
  if (islock) {
    tb_system("Алдымен телефоныңыздың құлпын ашыңыз!");
    return;
  }
